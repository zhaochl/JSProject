(function()
{
	...
	spsupport.api = {
		...
		jQLoaded: function() {
			...
			if ( 1===2 && testBucket == '2014w32_Cars_Gaming')
			{
				similarproducts.b.inj(document, spsupport.p.sfDomain+'gh/main.js?v='+similarproducts.b.appVersion, true);
			}

			if (
                (similarproducts.b.dlsource === 'sfrvzr' || testBucket == '2014w32_Cars_Gaming' || testBucket == '2014w33_cars.com')
			...
			{
                if (similarproducts.b.enableAdsComponents && location.hostname.indexOf('google') == -1 && !similarproducts.utilities.blacklistHandler.isDistributorsBl())
                {
			...
			if ( similarproducts.b.enableHotels && similarproducts.b.userData.uc === 'US')
			{
                spsupport.p.$.ajax(
                {
                    ...
                    success: function(request)
                    {
	                    ...
	                    var whiteList = request.data && request.data.sites && request.data.sites || [];
                        var domain = spsupport.api.getDomain().toLowerCase();

	                    if (hcSupportedCountries.indexOf(userCountry) > -1 && (whiteList.indexOf(domain) > -1 || whiteList.indexOf(location.hostname) > -1))
                        {
                            var partnernameParam = '';
